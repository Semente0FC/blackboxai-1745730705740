...
class EstrategiaTrading:
    def __init__(self, ativo, timeframe, lote, log_system):
        ...
        self.volume_threshold = 1.5  # Multiplicador para volume médio
    ...
    def analisar_e_operar(self):
        # Carregar mais dados históricos para cálculos mais precisos
        ...
        if sinal_compra:
            ...
            sl_distance = atr_atual * 1.5  # Stop Loss 1.5x ATR
            tp_distance = atr_atual * 2.5  # Take Profit 2.5x ATR
    ...
    def abrir_ordem(self, tipo_ordem, sl_distance, tp_distance):
        ...
        if resultado.retcode != mt5.TRADE_RETCODE_DONE:
            ...
            self.log_system.logar(f"✅ Ordem de {direcao} enviada com sucesso! Ticket: {self.ticket_atual}")
            self.log_system.logar(f"📊 SL: {sl:.5f} | TP: {tp:.5f}")
    ...
    def rsi(self, data, period=14):
        ...
        rs = avg_gain / np.where(avg_loss == 0, 0.000001, avg_loss)  # Evitar divisão por zero
        rsi = 100 - (100 / (1 + rs))
    ...
    def stochastic(self, high, low, close, period=14, k_smooth=3, d_smooth=3):
        ...
        k = k.rolling(window=k_smooth).mean()  # Smooth %K
        d = k.rolling(window=d_smooth).mean()  # %D is moving average of %K
    ...
    def momentum(self, data, period=10):
        ...
        momentum[:period] = momentum[period]  # Preencher valores iniciais
        return momentum
